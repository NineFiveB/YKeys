...
**The app has to hand the chord over.** `RegisterHotKey` is first-come, so if
the app you are signalling registers `alt+space` itself at startup — as YSpot
does out of the box — YKeys is refused that chord and skips the binding with a
line in the log. Nothing looks broken, because the chord still opens the app:
by *its* registration, not through YKeys. In YSpot that switch is Settings →
"Let YKeys hold the hotkey"; other apps will have their own.

Order does not matter: a skipped chord is retried every five seconds, so
whichever you do first, the binding takes hold within a few seconds of the
other app letting go, and says so in the log.
